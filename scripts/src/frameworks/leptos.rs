...
impl Framework for Leptos {
    ...
    fn lib_header(&self) -> Option<String> {
        Some(
            "\
            //! Leptos port of [Heroicons](https://heroicons.com/).\n\
            //!\n\
            //! Heroicons is a set of free MIT-licensed high-quality SVG icons for UI development.\n\
            //!\n\
            //! See [the Rust Heroicons book](https://heroicons.rustforweb.org/leptos.html) for more documenation.\n\
            "
            .to_owned()
        )
    }
    ...
    fn generate(&self, component_name: String, svg: String) -> Result<TokenStream, Box<dyn Error>> {
        ...
        let svg: TokenStream = svg
            .replacen("<svg", "<svg node_ref=node_ref class:heroicons=true ", 1)
