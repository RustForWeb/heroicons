...
const GIT_URL: &str = "https://github.com/tailwindlabs/heroicons.git";
const GIT_REF: &str = "v2.2.0";
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    for framework in &frameworks {
        ...
        framework.format(
            format!("heroicons-{}", framework.name()),
            Path::new("packages").join(framework.name()).join("src"),
        )?;

        framework.format(
            format!("heroicons-{}-book", framework.name()),
